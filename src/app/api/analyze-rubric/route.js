...
export async function POST(request) {
  try {
    ...
    // Truncate to avoid exceeding Gemini's context window (~8k chars keeps us well within token limits)
    ...
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          ...
          // Low temperature for consistent structured JSON output; 500 tokens is sufficient for the response schema
