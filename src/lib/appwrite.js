...
export async function predictTask(task, daysUntilDue = 7) {
  ...
  if (task.task_type === 'problem' && task.set_size > 0) {
    const problemTasks = completedTasks.filter(
      t => t.task_type === 'problem' && t.set_size > 0 && t.actual_time > 0
    );
    if (problemTasks.length > 0) {
      const avgPerProblem =
        problemTasks.reduce((sum, t) => sum + t.actual_time / t.set_size, 0) / problemTasks.length;
      avgPersonMinutes = task.set_size * avgPerProblem;
    }
  }

  // Motivation adjustment (low motivation = takes longer)
  // 50 is the baseline neutral motivation; deviations scale time up or down by up to ±50%
  const BASELINE_MOTIVATION = 50;
  const motivationFactor = 1 + (BASELINE_MOTIVATION - (task.motivation || BASELINE_MOTIVATION)) / 100;
